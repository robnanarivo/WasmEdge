...
2. Cost table
    ...
    /* Developers can set the costs of each instructions. The value not covered will be 0. */
...
The `VM` context supply the APIs to retrieve the instances.
...
/* Load WASM or compiled-WASM from file. */
Res = WasmEdge_LoaderParseFromFile(LoadCxt, &ASTCxt, "fibonacci.wasm");
if (!WasmEdge_ResultOK(Res)) {
  printf("Loading phase failed: %s\n", WasmEdge_ResultGetMessage(Res));
}
...
Every WASM modules should be validated before instantiation.
...
This object should work base on the `Store` context. For the details of `Store` context, please refer to the [next chapter](#Store).
...
[Store](https://webassembly.github.io/spec/core/exec/runtime.html#store) is the runtime structure for representation of all instances of `Function`s, `Table`s, `Memory`s, and `Global`s that have been allocated during the life time of the abstract machine.
...
1. List instances
    ...
    /* 
     * Developers can list the function instances of the registered modules via
     * the `WasmEdge_StoreListFunctionRegisteredLength()` and
     * `WasmEdge_StoreListFunctionRegistered()` APIs with the module name.
     */
    ```
...
2. Find instances
    ...
    /* 
     * Developers can find the function instances of the registered modules via
     * the `WasmEdge_StoreFindFunctionRegistered()` API with the module name.
     */
    ```
...
3. List registered modules
    ...
    /* Developers can list the exported functions of module with name `ModNames[I]` by the WasmEdge_StoreListFunctionRegistered() API. */
    ```
...
1. Import object context

2. Host function allocation
